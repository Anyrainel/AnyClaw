...
# package-skills.sh — Package AnyRaven skill files for different agent platforms.
#
# Usage:
...
cmd_openclaw() {
  local source="anyraven-server/skills/raw"
...
cmd_claude_code() {
  ...
  # 2. Generate or update CLAUDE.md with AnyRaven Agent Instructions block
  local claude_md="$project_dir/CLAUDE.md"
  local sentinel_begin="<!-- anyraven:begin -->"
  local sentinel_end="<!-- anyraven:end -->"

  local block
  block=$(cat <<'AGENTBLOCK'
<!-- anyraven:begin -->
## AnyRaven Agent Instructions

### Skills (slash commands)

Use these skills during every task:

- `/anyraven-developer-loop` — Use this as the top-level state machine for every request: clarify only requirements, break work into commit-sized vertical slices, test each slice, publish, then report.
- `/anyraven-build-feature` — Use this for the concrete AnyRaven implementation steps inside each feature request.
- `/anyraven-style-guide` — Follow this for all frontend code. Tailwind v4 conventions, component patterns, voice and tone.
- `/anyraven-canonical-example` — Read `dev/_examples/welcome.tsx` before writing any new frontend code. Every task, every time.
- `/anyraven-refactor` — Run a cleanup pass every 5 deployments or when complexity grows.
- `/anyraven-describe-version` — Write a non-technical version description for every deployment.
...
AGENTBLOCK
  ...
  echo "Updated $claude_md with AnyRaven Agent Instructions"
}

cmd_generic() {
  local source="anyraven-server/skills/raw"
  local out="anyraven-server/skills/raw/system-prompt.txt"
  ...
  {
    echo "# AnyRaven Agent System Prompt"
