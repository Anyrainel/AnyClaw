...
1. Call `anyraven_list_versions` and read recent deployment descriptions.
...
Use `anyraven-describe-version` to write the version description, then call
`anyraven_deploy`. The deploy step is the authoritative publish gate.
...
- Never edit `/.anyraven/`; it is infrastructure.
