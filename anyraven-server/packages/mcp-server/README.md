...
| Tool | Description |
|---|---|
| `anyraven_deploy` | Validate worktree, snapshot DB, merge to main, promote to prod, restart app backend |
| `anyraven_rollback` | Restore DB snapshot and revert code to a prior version tag |
| `anyraven_snapshot_db` | Create a labelled gzip SQLite snapshot |
| `anyraven_list_versions` | Return deployment history |
| `anyraven_create_collection` | Create a PocketBase collection (auto-snapshots first) |
| `anyraven_ask_user` | Post a clarifying question to the mobile app; block until answered |
| `anyraven_update_progress` | Fire-and-forget progress update to the mobile app |
...
```typescript
import { mountMcp, registerTaskToken, revokeTaskToken } from "@anyraven/mcp-server";
...
- `@anyraven/shared` — paths, deploy/rollback managers, snapshot manager, version store
