...
async function buildTestServer(
  ...
  tmpDir = await mkdtemp(join(tmpdir(), "anyraven-e2e-"));
  // The MCP server auth module reads ANYRAVEN_DATA_ROOT from process.env
  process.env.ANYRAVEN_DATA_ROOT = tmpDir;
