...
describe("checkSkillCompatibility", () => {
  it("passes when all skills are compatible", () => {
    const skills = new Map([
      ["anyraven-build-feature", VALID_SKILL_RAW],
    ]);
  ...
  it("rejects when a skill version is below minSkillVersion", () => {
    const skills = new Map([
      ["anyraven-old-skill", OLD_SKILL_RAW],
    ]);
    ...
    if (!result.ok) {
      expect(result.error).toContain("anyraven-old-skill");
  ...
  it("rejects when server version is below skill's min_server_version", () => {
    const skills = new Map([
      ["anyraven-demanding-skill", DEMANDING_SKILL_RAW],
    ]);
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("anyraven-demanding-skill");
  ...
  it("reports the name of the offending skill in the error", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.error).not.toContain("anyraven-build-feature");
    }
  });
