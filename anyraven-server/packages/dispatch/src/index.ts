...
function makePaths(dataRoot: string) {
  return new AnyRavenPaths(dataRoot);
}
...
function getPocketBaseAdmin(dataRoot: string): PocketBase {
  const envToken =
    ...
    process.env.ANYRAVEN_PB_TOKEN;
  const token =
    envToken ??
    readFileSync(path.join(dataRoot, ".anyraven", "pb-token"), "utf8").trim();
  ...
  app.use((req, res, next) => {
    ...
    if (typeof origin === "string" && isAllowedCorsOrigin(origin)) {
      ...
      res.setHeader(
        ...
        "authorization,content-type,x-anyraven-task-id",
      );
      res.setHeader(
...
function makeAdapter(config: DispatchConfig, repo: TasksRepo, _paths: AnyRavenPaths): AgentAdapter {
  switch (config.adapter) {
    case "openclaw": {
      const gatewayUrl = process.env.OPENCLAW_GATEWAY_URL ?? "ws://127.0.0.1:18789/gateway";
      const token = process.env.OPENCLAW_TOKEN ?? "";
      const workspace = process.env.OPENCLAW_WORKSPACE ?? "anyraven-test";
...
export async function restartSupervisorProgram(
  ...
  if (process.env.ANYRAVEN_DISABLE_SUPERVISOR_RESTART === "1") {
    return;
  }
...
function makeManagers(paths: AnyRavenPaths) {
...
export async function buildServer(opts: BuildServerOptions) {
  const dataRoot = opts.dataRoot ?? process.env.ANYRAVEN_DATA_ROOT ?? "/data";
  ...
  const buildSystemContext = async (taskId: string): Promise<SystemContext> => {
    ...
    const allowedToolsEnv = process.env.ANYRAVEN_ALLOWED_TOOLS;
    ...
    return {
      ...
      systemPrompt: process.env.ANYRAVEN_SYSTEM_PROMPT ?? "",
      allowedTools,
    };
  };
  ...
  buildApp(app, {
    ...
    autoProcessQueue: process.env.ANYRAVEN_DISABLE_TASK_AUTORUN !== "1",
  });
