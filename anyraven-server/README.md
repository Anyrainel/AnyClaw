...
```bash
npm run -w @anyraven/shared test
npm run -w @anyraven/dispatch build
```

## Filesystem Layout

All persistent runtime state lives under `/data` (override with `ANYRAVEN_DATA_ROOT` in tests):
...
└── .anyraven/                    Secrets: pb-token, mcp-tokens/, logs/
```
