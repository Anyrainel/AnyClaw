#!/usr/bin/env bash
# AnyClaw One-Command Installer
# Usage: curl -fsSL https://get.anyclawapp.com | bash
set -euo pipefail

ANYCLAW_VERSION="${ANYCLAW_VERSION:-latest}"
INSTALL_DIR="${ANYCLAW_DIR:-$HOME/.anyclaw-host}"
RELEASE_BASE="${ANYCLAW_RELEASE_BASE:-https://releases.anyclawapp.com}"
CONTAINER_NAME="anyclaw"

# ── Helpers ──────────────────────────────────────────────────────────

info()  { echo "[AnyClaw]  $*"; }
warn()  { echo "[AnyClaw]  WARNING: $*" >&2; }
error() { echo "[AnyClaw]  ERROR: $*" >&2; exit 1; }
...
check_prerequisites() {
  ...
  if [ "$os" = "unknown" ]; then
    error "Unsupported operating system: $(uname -s). AnyClaw requires Linux or macOS."
  fi
  ...
  if [ "$mem_kb" -gt 0 ] && [ "$mem_kb" -lt 2097152 ]; then
    warn "Less than 2 GB RAM detected (${mem_kb} kB). AnyClaw may run slowly."
  fi
  ...
  if [ "${avail_kb:-0}" -gt 0 ] && [ "$avail_kb" -lt 5242880 ]; then
    warn "Less than 5 GB disk space available. AnyClaw needs room for images and data."
  fi
...
setup_install_dir() {
  ...
  if [ ! -f "$INSTALL_DIR/docker-compose.yml" ]; then
    ...
    curl -fsSL "$RELEASE_BASE/$ANYCLAW_VERSION/docker-compose.yml" \
  ...
  if [ ! -f "$INSTALL_DIR/.env" ]; then
    ...
    curl -fsSL "$RELEASE_BASE/$ANYCLAW_VERSION/env.template" \
    ...
    sed -i.bak "s|ANYCLAW_USER_TOKEN=.*|ANYCLAW_USER_TOKEN=$user_token|" "$INSTALL_DIR/.env"
...
prompt_llm_key() {
  ...
  # Store for phase 5
  ANYCLAW_LLM_PROVIDER="$provider"
  ANYCLAW_LLM_KEY="$api_key"
}
...
bootstrap_pocketbase() {
  ...
  docker compose exec "$CONTAINER_NAME" bash -c "
    mkdir -p /data/.anyclaw
    echo '$admin_password' > /data/.anyclaw/pb-admin
    chmod 0600 /data/.anyclaw/pb-admin
    chown anyclaw-infra:anyclaw-infra /data/.anyclaw/pb-admin 2>/dev/null || true
    echo '$long_token' > /data/.anyclaw/pb-token
    chmod 0600 /data/.anyclaw/pb-token
    chown anyclaw-infra:anyclaw-infra /data/.anyclaw/pb-token 2>/dev/null || true
  "

  # Export for use by seed script
  ANYCLAW_PB_TOKEN="$long_token"
...
store_llm_key() {
  ...
  docker compose exec "$CONTAINER_NAME" bash -c "
    if [ ! -f /data/.anyclaw/master.key ]; then
      openssl rand -base64 32 > /data/.anyclaw/master.key
      chmod 0600 /data/.anyclaw/master.key
      chown anyclaw-infra:anyclaw-infra /data/.anyclaw/master.key 2>/dev/null || true
    fi
  "
  ...
  response=$(docker compose exec "$CONTAINER_NAME" \
    curl -fsS -X POST http://127.0.0.1:4100/internal/api-keys \
      -H 'Content-Type: application/json' \
      -d "{\"provider\":\"$ANYCLAW_LLM_PROVIDER\",\"key\":\"$ANYCLAW_LLM_KEY\"}" \
  ) || error "Failed to store LLM key via dispatch endpoint."

  # Clear the key from memory (best effort in bash)
  ANYCLAW_LLM_KEY=""

  info "  LLM key stored securely (provider: $ANYCLAW_LLM_PROVIDER)."
}
...
package_skills() {
  ...
  docker compose exec "$CONTAINER_NAME" \
    PB_URL=http://127.0.0.1:8090 PB_TOKEN="${ANYCLAW_PB_TOKEN:-}" \
    node /.anyclaw/scripts/seed-welcome-collection.js || warn "Tip seeding failed (non-fatal)."
}
...
main() {
  ...
  info ""
  info "  Open the AnyClaw mobile app to get started!"
}
