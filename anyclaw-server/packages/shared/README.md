...
Shared library consumed by all anyclaw-server packages. Provides cryptographic primitives, filesystem path resolution, git worktree management, database snapshot management, version store, and deploy/rollback orchestration.

## Exports

| Module | Exports | Purpose |
|---|---|---|
| `paths.ts` | `AnyClawPaths` | Resolve all well-known `/data/*` paths from a configurable root |
...
const paths = new AnyClawPaths(process.env.ANYCLAW_DATA_ROOT ?? "/data");
```

All classes accept an `AnyClawPaths` instance so tests can override the data root via `ANYCLAW_DATA_ROOT`.

## `AnyClawPaths` Reference
