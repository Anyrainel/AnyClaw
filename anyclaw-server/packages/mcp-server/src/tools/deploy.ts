...
const defaultMgr: () => DeployManagerLike = () =>
  (require("@anyclaw/shared") as any).deployManager;
...
export function registerDeploy(server: any, ctx: { taskId: string }) {
  server.registerTool(
    "anyclaw_deploy",
    {
      ...
      description:
        "Validate, snapshot, commit, merge to main, promote, restart logic service. REQUIRES a version description a non-technical user can understand.",
