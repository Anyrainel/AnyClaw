/**
 * Architecture boundary tests for the anyclaw-server monorepo.
 *
...
function anyclawImports(files: string[]): Record<string, string[]> {
  const violations: Record<string, string[]> = {};
  for (const file of files) {
    const content = readFileSync(file, "utf8");
    const found = [...content.matchAll(/from ["'](@anyclaw\/[^"']+)["']/g)].map(m => m[1]!);
...
describe("shared — foundation, no upward deps", () => {
  it("has no @anyclaw/* imports", () => {
    const violations = anyclawImports(walkTs(pkgSrc("shared")));
...
describe("tunnel-manager — leaf service", () => {
  it("does not import @anyclaw/dispatch or @anyclaw/mcp-server", () => {
    const violations = anyclawImports(walkTs(pkgSrc("tunnel-manager")));
...
describe("mcp-server — mounted by dispatch, never imports it", () => {
  it("does not import @anyclaw/dispatch", () => {
    const violations = anyclawImports(walkTs(pkgSrc("mcp-server")));
    const forbidden = Object.entries(violations).flatMap(([f, imps]) =>
      imps.filter(i => i === "@anyclaw/dispatch").map(i => `${f}: ${i}`)
    );
    expect(forbidden).toEqual([]);
  });
});
...
describe("frontend-template — standalone Vite project", () => {
  it("has no @anyclaw/* imports", () => {
    const violations = anyclawImports(walkTs(pkgSrc("frontend-template")));
    expect(violations).toEqual({});
  });
});
