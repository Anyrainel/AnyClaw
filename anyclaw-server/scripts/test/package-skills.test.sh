...
# Run:  bash anyclaw-server/scripts/test/package-skills.test.sh
...
echo "old content" > "$TMPDIR_OC/skills/anyclaw-build-feature.md"
bash "$PACKAGE_SKILLS" openclaw --source "$SKILLS_SRC" --dest "$TMPDIR_OC/skills"
assert_not_contains "openclaw: pre-existing overwritten" "$TMPDIR_OC/skills/anyclaw-build-feature.md" "old content"
...
assert_contains "claude-code: lists refactor" "$TMPDIR_CC/CLAUDE.md" "anyclaw-refactor"
assert_contains "claude-code: lists describe-version" "$TMPDIR_CC/CLAUDE.md" "anyclaw-describe-version"

# Test 4: CLAUDE.md lists all 7 MCP tools
assert_contains "claude-code: lists anyclaw_deploy" "$TMPDIR_CC/CLAUDE.md" "anyclaw_deploy"
assert_contains "claude-code: lists anyclaw_rollback" "$TMPDIR_CC/CLAUDE.md" "anyclaw_rollback"
assert_contains "claude-code: lists anyclaw_snapshot_db" "$TMPDIR_CC/CLAUDE.md" "anyclaw_snapshot_db"
assert_contains "claude-code: lists anyclaw_create_collection" "$TMPDIR_CC/CLAUDE.md" "anyclaw_create_collection"
assert_contains "claude-code: lists anyclaw_ask_user" "$TMPDIR_CC/CLAUDE.md" "anyclaw_ask_user"
assert_contains "claude-code: lists anyclaw_update_progress" "$TMPDIR_CC/CLAUDE.md" "anyclaw_update_progress"
assert_contains "claude-code: lists anyclaw_list_versions" "$TMPDIR_CC/CLAUDE.md" "anyclaw_list_versions"

# Test 5: Re-run does not duplicate block
bash "$PACKAGE_SKILLS" claude-code --project-dir "$TMPDIR_CC"
sentinel_count=$(grep -c "<!-- anyclaw:begin -->" "$TMPDIR_CC/CLAUDE.md" || true)
...
assert_contains "claude-code: has block after re-run" "$TMPDIR_CC/CLAUDE.md" "<!-- anyclaw:begin -->"
...
assert_contains "generic: contains refactor" "$TMPDIR_GEN/system-prompt.txt" "# anyclaw-refactor"
assert_contains "generic: contains describe-version" "$TMPDIR_GEN/system-prompt.txt" "# anyclaw-describe-version"

# Test 2: No frontmatter in output
assert_not_contains "generic: no frontmatter" "$TMPDIR_GEN/system-prompt.txt" "^skill_version:"

# Test 3: Preamble present
assert_contains "generic: preamble line 1" "$TMPDIR_GEN/system-prompt.txt" "# AnyClaw Agent System Prompt"
